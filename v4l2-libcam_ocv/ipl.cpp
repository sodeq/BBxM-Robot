 ...
 * TODO: need better MEMORY MANAGEMENT!!!
...
unsigned char* IMAGING::GenerateRGB() {
	// allocate memory!
	// we need to free the memory buffer!
	if(rgb_data != NULL) free(rgb_data);
...
void IMAGING::Split(myImage *M) const {
	if(M != NULL) {
		free(M->data);
		free(M);
	}
	M = (myImage *)malloc(sizeof(myImage)*nc);
	for(int idx = 0; idx < nc; ++idx) {
		M[idx].data = (unsigned char *)malloc(sizeof(unsigned char)*w*h);
		M[idx].w = w;
		M[idx].h = h;
		M[idx].ncParent = nc;
		for(int x = 0; x < w; ++x)
			for(int y = 0; y < h; ++y)
				M[idx].data[(y*h + x)] = p[(y*h + x)*nc + idx];
	}
}
