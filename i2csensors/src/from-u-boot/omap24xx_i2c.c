...
static int fd = -1;
static volatile struct i2c *i2c_base = NULL;

static unsigned int bus_initialized[I2C_BUS_MAX];
static unsigned int current_bus = 1;
...
{ 
  ...
  uint32_t scll, sclh;
	...
	if ((speed != OMAP_I2C_STANDARD) &&
		...
		fprintf(stderr, "Error : I2C unsupported speed %d\n", speed);
		return;
	}
  ...
  if(i2c_base == NULL)
  {
    ...
    i2c_base = 
      (volatile struct i2c *)mmap(NULL, sizeof(struct i2c), 
	...
	psc -= 1;
	if (psc < I2C_PSC_MIN) {
		fprintf(stderr, "Error : I2C unsupported prescalar %d\n", psc);
		return;
	}
	...
	if (speed == OMAP_I2C_HIGH_SPEED) {
		...
		if (((fsscll < 0) || (fssclh < 0)) ||
			...
			fprintf(stderr, "Error : I2C initializing first phase clock\n");
			return;
		}
		...
		if (((fsscll < 0) || (fssclh < 0)) ||
		    ((fsscll > 255) || (fssclh > 255))) {
			fprintf(stderr, "Error : I2C initializing second phase clock\n");
			return;
		}
		...
		if (((fsscll < 0) || (fssclh < 0)) ||
		    ((fsscll > 255) || (fssclh > 255))) {
			fprintf(stderr, "Error : I2C initializing clock\n");
			return;
		}
	...
	if(readw(&i2c_base->con) & I2C_CON_EN)
  {
	...
	while (!(readw(&i2c_base->syss) & I2C_SYSS_RDONE) && timeout--) {
		if (timeout <= 0) {
			fprintf(stderr, "ERROR: Timeout in soft-reset\n");
			return;
		}
		udelay(1000);
	}
